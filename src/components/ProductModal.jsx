...
export default function ProductModal({ product, onClose }) {
  ...
  const allPhotos = product.photos?.length > 0 ? product.photos : [product.photo];
        ...
        <div style={styles.layout} className="modal-layout">
          ...
          <div style={styles.imageSection}>
            <div style={styles.mainImageWrap}>
              {product.photo ? (
                <img src={allPhotos[activePhoto] || product.photo} alt={product.name} style={styles.mainImage} />
              ) : (
              ...
              <div style={styles.thumbs}>
                {allPhotos.map((ph, i) => (
                  <button
                    ...
                    style={{ ...styles.thumbBtn, outline: activePhoto === i ? "2px solid var(--gold)" : "none" }}
                  >
          ...
          <div style={styles.infoSection}>
            ...
            <div style={styles.specsGrid}>
              {[
                { label: "Contents", value: product.sticks },
                { label: "Weight",   value: product.weight },
                { label: "Burn Time",value: product.burn },
                { label: "Rating",   value: `${product.rating} — ${product.reviews}` },
              ].map(({ label, value }) => (
...
const styles = {
  ...
  modal: {
    ...
    background: "var(--white)", zIndex: 1400,
    width: "min(900px, 95vw)", maxHeight: "90vh",
    overflowY: "auto", boxShadow: "0 25px 80px rgba(0,0,0,0.5)",
  },
  closeBtn: {
    position: "absolute", top: 15, right: 20,
    background: "none", border: "none", fontSize: "1.3rem",
    cursor: "pointer", color: "var(--muted)", zIndex: 1,
  },
  ...
  thumbBtn: { border: "none", cursor: "pointer", padding: 0, borderRadius: 2, overflow: "hidden" },
  thumbImg: { width: 52, height: 52, objectFit: "cover", display: "block" },
  infoSection: { padding: "2.5rem", display: "flex", flexDirection: "column", gap: "1rem" },
  scent: {
  ...
  name: {
    ...
    fontWeight: 600, color: "var(--deep)", lineHeight: 1.3,
  },
  priceRow: { display: "flex", alignItems: "baseline", gap: "0.8rem" },
  price: { fontFamily: "'Cinzel Decorative', serif", color: "var(--gold)", fontSize: "1.4rem" },
  originalPrice: { color: "var(--muted)", textDecoration: "line-through", fontSize: "0.95rem" },
  longDesc: {
  ...
  specsGrid: {
    ...
    padding: "1rem", background: "var(--smoke)",
  },
  specItem: { display: "flex", flexDirection: "column", gap: 2 },
  specLabel: { fontSize: "0.6rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--muted)" },
  specValue: { fontSize: "0.85rem", color: "var(--text)", fontWeight: 500 },
  actions: { display: "flex", gap: "1rem", alignItems: "center", marginTop: "0.5rem" },
  qtyRow: { display: "flex", alignItems: "center", gap: 8 },
  qtyBtn: {
    background: "rgba(58,37,16,0.1)", border: "none", width: 32, height: 32,
    cursor: "pointer", fontSize: "1.1rem",
  },
  qtyNum: { fontSize: "1rem", fontWeight: 500, minWidth: 24, textAlign: "center" },
  addBtn: {
    flex: 1, background: "var(--deep)", color: "var(--gold)", border: "none",
    padding: "1rem 1.5rem", cursor: "pointer", fontFamily: "'Jost', sans-serif",
    fontSize: "0.8rem", letterSpacing: "0.15em", textTransform: "uppercase", transition: "all 0.3s",
  },
